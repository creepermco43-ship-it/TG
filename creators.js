...
function creatorImg(c,size){
  const s=size==='lg'?'110px':'68px', f=size==='lg'?'2.2rem':'1.5rem';
  const ini=c.gameName.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase();
  return`<img src="${c.img}" style="width:${s};height:${s};border-radius:50%;object-fit:cover;border:2px solid #333;display:block;transition:border-color .3s" onerror="this.onerror=null;var b=this.src,n=b.split('/').pop().split('.')[0],e=['.jpg','.jpeg','.png','.webp'],i=e.indexOf('.'+b.split('.').pop());if(i<e.length-1){this.src='images/'+n+e[i+1]}else if(!b.endsWith('temp.jpg')){this.src='images/temp.jpg'}else{this.style.display='none';var p=this.nextSibling;while(p&&p.style===undefined)p=p.nextSibling;if(p)p.style.display='flex'}" alt="${c.gameName}"><div style="width:${s};height:${s};border-radius:50%;border:2px solid #222;background:linear-gradient(135deg,#1a1a1a,#2a2a2a);display:none;align-items:center;justify-content:center;font-family:'Bebas Neue',sans-serif;font-size:${f};color:var(--orange)">${ini}</div>`;
}
function links(c,sm){
